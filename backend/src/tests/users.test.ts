...
async function loginAsAdmin() {
  await createUser("Admin", "password", "Admin", "Admin", "Admin", "https://example.com/profile.jpg");
...
async function loginAsMom(){
  await createUser("Mom", "password", "Mom", "Mom", "Mom", "https://example.com/profile.jpg");
  const response = await request(app).post("/api/login").send({
    username: "Mom",
    password: "password"
  });
...
function createTestImage(): string {
  const testImagePath = path.join(__dirname, '../../public/images/test-image.jpg');
...
describe("Test getting user endpoint", () => {
  test("Should return user with admin role", async () => {
    const cookie = await loginAsAdmin();
    const response = await request(app).get("/api/users/1").set("Cookie", cookie);
    expect(response.status).toBe(201);
...
describe("Test login endpoint", () => {
  test("Should login successfully", async () => {
    await createUser("TestUser", "password", "Test", "User", "Volunteer", "https://example.com/profile.jpg");
    const response = await request(app).post("/api/login").send({
      username: "TestUser",
      password: "password"
    });
  ...
  test("Should reject invalid credentials", async () => {
    ...
    expect(response.status).toBe(401);
    expect(response.body.message).toBe("No account with username InvalidUser exists");
  });
...
describe("Test delete user endpoint", () => {
  test("Should delete user with admin role", async () => {
    const cookie = await loginAsAdmin();
    const response = await request(app).delete("/api/users/1").set("Cookie", cookie);
    expect(response.status).toBe(201);
...
describe("Test verify session endpoint", () => {
  test("Should verify valid session", async () => {
    const cookie = await loginAsAdmin();
    const response = await request(app).post("/api/verify-session").set("Cookie", cookie);
    expect(response.status).toBe(201);
