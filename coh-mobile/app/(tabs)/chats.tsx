...
export default function Chats() {
  ...
  async function onSubmit(id: Number) {
    ...
    const chatsResorted = [
      selectedChat,
      ...chats.filter((chat) => chat.chatId !== id),
    ]
  ...
  function getChatsList() {
    if (chats.length === 0 && role === 'Mom') {
      return (
        <Text className='text-7 m-2 text-center text-blue-400'>
          {' '}
          You do not have any open chats
        </Text>
      )
    }
    const chatsList = chats.map((chat: any, index: any) => {
      ...
      return (
        <Pressable
          onPress={() => onSubmit(chat.id)}
          key={index}
          className='items-center justify-start border-2 border-blue-300 rounded-md bg-gray-200 px-2 py-1 mt-3 flex-row'>
          {/* <ImageWithRedirect resizeMode="cover" className="w-6 h-6 rounded-full" source={chat.otherProfileLink}/> */}
          <Image
            source={{ uri: chat.otherProfileLink }}
            className='w-7 h-7 rounded-full'
            resizeMode='cover'
          />
          <View className='ml-2 mr-3 justify-between'>
            <Text className='font-primary text-blue-600 text-4 max-w-11'>
              {chat.otherName}
            </Text>
            <Text className='font-primary text-gray-500 text-2'>
              {displayMessage}
            </Text>
          </View>
          <Text className='self-start ml-auto'>
            {month} {day}
          </Text>
        </Pressable>
      )
  ...
  return (
    <SafeAreaView className='flex-1'>
        <View className='px-2 text-gray-200 items-center justify-center py-5'>
          <Text className='font-primary text-pink-400 text-7 mb-7'>
            {' '}
            All Chats
          </Text>
